...
VALUES = {"person": PEOPLE_ZH + PEOPLE_EN, "email": EMAILS, "phone": PHONES, "idnum": IDNUMS, "card": CARDS,
...
def run(cfg: Config, entities: Callable[[list[str]], list[list[dict[str, Any]]]] | None,
        app_names: list[str]) -> dict[str, Any]:
    items = corpus()
    per_kind: dict[str, list[int]] = {}
    leaks: list[dict[str, str]] = []
    for kind, value, text in items:
        r = Redactor(cfg, entities=entities, protect=lambda: app_names)   # a fresh table per text: nothing learned across
        out = r.text(text)
    ...
    for name in app_names[:80]:
        for t in ("打开 {v}", "Open {v}", "切换到 {v}"):
            ...
            out = Redactor(cfg, entities=entities, protect=lambda: app_names).text(text)
            if name not in out:
    ...
    for word in UI_WORDS:                         # ordinary interface words must reach the decider as they are
        out = Redactor(cfg, entities=entities, protect=lambda: app_names).text(word)
        if out != word:
